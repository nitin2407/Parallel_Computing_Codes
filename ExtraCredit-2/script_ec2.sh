...
mpic++ -std=c++11 -o mpi mpi_nqueens.cpp
...
do
	...
	do
		...
		par=$(mpirun -n $threads ./mpi $n)
		par=$( printf "%.10f" $par )
		echo $threads $(echo "$seq/$par"|bc -l)>>data/outEC2_mpi_$n.dat
	done
	gnuplot -e "inp = 'data/outEC2_openmp_$n.dat'; out='plots/plotEC2_openmp_$n.png'; titl='SpeedUp Vs Processor Plot for n=$n'" plot.sh
	gnuplot -e "inp = 'data/outEC2_mpi_$n.dat'; out='plots/plotEC2_mpi_$n.png'; titl='SpeedUp Vs Processor Plot for n=$n'" plot.sh
done
set +x
